...
from .utils import RAW_DATASETS_DIR, RESULTS_DIR
...
def _resolve_submetrics_config(
    submetrics_config: Union[Dict[str, List[str]], str],
    data_dir: str,
) -> Dict[str, List[str]]:
    """
    Resolves a dataset's submetrics config, loading it from a sibling file
    if given as a filename instead of an inline dict.

    Some submetrics (e.g. large per-document label lists) are impractical
    to inline in a dataset's committed config.json. In that case,
    config.json's "submetrics" value is a filename (e.g.
    "submetrics.json"), resolved relative to the dataset's raw data
    directory, and regenerated by that dataset's prep script alongside its
    raw data rather than committed to the repo.

    Args:
        submetrics_config: Either the submetrics dict directly, or the
            filename of a JSON file (relative to the dataset's data_dir)
            containing it.
        data_dir: The dataset's "data_dir" config value.

    Returns:
        The resolved submetrics dict, mapping submetric name to label list.
    """
    if isinstance(submetrics_config, str):
        submetrics_path = os.path.join(RAW_DATASETS_DIR, data_dir, submetrics_config)
        with open(submetrics_path) as f:
            return json.load(f)
