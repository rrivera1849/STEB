...
from sklearn.metrics import roc_curve, roc_auc_score, v_measure_score, pairwise_distances

def l2_normalize(
    embeddings: np.ndarray,
) -> np.ndarray:
    """L2-normalize the last dimension of embeddings, safely.
       RRS - Arguably, this shouldn't go here...
    """
...
def calculate_pair_classification_metrics(
    ...
    if fpr_thresholds is None:
        fpr_thresholds = [0.01, 0.05, 0.10, 0.20, 0.30, 0.50]
    ...
    for fpr in fpr_thresholds:
        auc_threshold = roc_auc_score(y_true, y_score, max_fpr=fpr)
        metrics["auc@{:.2f}".format(fpr)] = auc_threshold
